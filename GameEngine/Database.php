...
class MYSQLi_DB implements IDbConnection
{
    ...
    function getVilWrefs($coordinatesArray)
    {
        ...
        if (!is_array($coordinatesArray[0]))
    ...
    function addResourceFields($vids, $types, $buildingsArray = null)
    {
        ...
        foreach ($vids as $index => $vid) {
            ...
            switch ($types[$index]) {
                case 1:
                    $stringValues .= "4,4,1,4,4,2,3,4,4,3,3,4,4,1,4,2,1,2";
                    break;
                case 2:
                    $stringValues .= "3,4,1,3,2,2,3,4,4,3,3,4,4,1,4,2,1,2";
                    break;
                case 3:
                    $stringValues .= "1,4,1,3,2,2,3,4,4,3,3,4,4,1,4,2,1,2";
                    break;
                case 4:
                    $stringValues .= "1,4,1,2,2,2,3,4,4,3,3,4,4,1,4,2,1,2";
                    break;
                case 5:
                    $stringValues .= "1,4,1,3,1,2,3,4,4,3,3,4,4,1,4,2,1,2";
                    break;
                case 6:
                    $stringValues .= "4,4,1,3,4,4,4,4,4,4,4,4,4,4,4,2,4,4";
                    break;
                case 7:
                    $stringValues .= "1,4,4,1,2,2,3,4,4,3,3,4,4,1,4,2,1,2";
                    break;
                case 8:
                    $stringValues .= "3,4,4,1,2,2,3,4,4,3,3,4,4,1,4,2,1,2";
                    break;
                case 9:
                    $stringValues .= "3,4,4,1,1,2,3,4,4,3,3,4,4,1,4,2,1,2";
                    break;
                case 10:
                    $stringValues .= "3,4,1,2,2,2,3,4,4,3,3,4,4,1,4,2,1,2";
                    break;
                case 11:
                    $stringValues .= "3,1,1,3,1,4,4,3,3,2,2,3,1,4,4,2,4,4";
                    break;
                case 12:
                    $stringValues .= "1,4,1,1,2,2,3,4,4,3,3,4,4,1,4,2,1,2";
                    break;
                default:
                    $stringValues .= "4,4,1,4,4,2,3,4,4,3,3,4,4,1,4,2,1,2";
            }
    ...
    function getVillageState($wref, $use_cache = true)
    {
        ...
        if ($use_cache && ($cachedValue = self::returnCachedContent(self::$villageFieldsCacheByWorldID, $wref)) && !is_null($cachedValue)) {
            ...
            return ($vil && ($vil['occupied'] != 0 || $vil['oasistype'] != 0));
        }
    }
    ...
    function getProfileVillages($uid, $mode = 0, $use_cache = true)
    {
        ...
        switch ($mode) {
            // by owner ID
            case 0:
                $q = "SELECT * FROM " . TB_PREFIX . "vdata WHERE owner IN(" . implode(', ', $uid) . ") ORDER BY pop DESC";
                break;
    ...
    ***************************/
    ...
    function getBuildingTime($id, $tid, $plus, $wref, $buildingArray)
    {
        ...
        if ($tid == 15) {
            if ($mainBuilding == 0)
                return round($dataArray[$buildingArray['f' . $id] + $plus]['time'] / SPEED * 5);
            else
                return round($dataArray[$buildingArray['f' . $id] + $plus]['time'] / SPEED);
        } else {
            if ($mainBuilding > 0) {
    ...
    function removeBuilding($d, $tribe, $wid, $fieldsArray = [])
    {
        ...
        if (empty($fieldsArray))
            $fieldsArray = $this->getResourceLevel($wid);
        ...
        foreach ($jobs as $job) {
            ...
            if (!empty($jobToDelete) && $job['loopcon'] == 1 && ($tribe != 1 || ($tribe == 1 && (($jobToDelete['field'] <= 18 && $job['field'] <= 18) || ($jobToDelete['field'] >= 19 && $job['field'] >= 19))))) {
                ...
                if ($sameBuilding && $canBeRemoved)
                    $canBeRemoved = !$sameBuilding;
    ...
    function checkEmbassiesAfterBattle($vid, $current_level, $use_cache = true)
    {
        ...
        if ($allianceStatus === false)
            ...
            return ''; // all is good, no need to append additional alliance-related text
    }
    ...
    function totalMerchantUsed($vid, $use_cache = true)
    {
        ...
        self::$merchantsUseCountCache[$vid] = mysqli_fetch_array(
            mysqli_query(
                $this->dblink,
                '
            SELECT
    ...
    function addUnits($vid, $troopsArray = null)
    {
        ...
        foreach ($vid as $index => $vidValue)
            $vid[$index] = (int) $vidValue . ($troopsArray != null ? "," . implode(",", $values[$index]) : "");
    ...
    function getArtifactsValueInfluence($uid, $vid, $kind, $multiplicand, $round = true)
    {
        ...
        if (in_array($kind, [2, 4, 5]))
            $foolEffect = 1 / $foolEffect;
    ...
    function resetPassword($uid, $cpw)
    {
        ...
        if (!empty($dbarray)) {
            if (!$this->updateUserField($uid, 'password', password_hash($dbarray['npw'], PASSWORD_BCRYPT, ['cost' => 12]), 1))
                return false;
